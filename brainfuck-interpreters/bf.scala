...
@main def main(): Unit = {
    ...
    // val program = "++>+++++[<+>-]++++++++[<++++++>-]<."
    // val resultMemory = run(program)
    // println(resultMemory)

    // val helloWorld = "++++++++++[>+++++++>++++++++++>+++>+<<<<-]>++.>+.+++++++..+++.>++.<<+++++++++++++.>++++.---------------.+++++.------.--------.>+."
    // val resultMemory = run(helloWorld)
    // println(resultMemory)
    ...
    val bfProgram = generate("ABC".toList)
    println(bfProgram)
...
def compute(prog: String, pc: Int, mp: Int, mem: Mem) : Mem =   {
    if (pc < 0 || pc>= prog.length) mem
        ...
        case '+' =>
            ...
            compute(prog, pc+1, mp, write(mem, mp, incValue))
        case '-' =>
            val decValue = sread(mem, mp) - 1
            compute(prog, pc+1, mp, write(mem, mp, decValue))
        ...
        case ',' =>
            ...
            compute(prog, pc+1, mp, write(mem, mp, input))
