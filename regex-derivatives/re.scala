...
def denest(rs: List[Rexp]) : List[Rexp] = rs match {
  ...
  case ALTs(content) :: rest => content ::: denest(rest)
  case r :: rest => r :: denest(rest)
  // case Nil => Nil
  // case ZERO :: rest => denest(rest)  
  // case ALTs(content) :: rest => denest(content) ::: denest(rest)  
  // case r :: rest => r :: denest(rest)
}
...
def flts(rs: List[Rexp], acc: List[Rexp] = Nil) : List[Rexp] = rs match  {
  ...
  case SEQs(content) :: rest => flts(rest, acc:::content)
  case r :: rest => flts(rest, acc :+ r)
}
