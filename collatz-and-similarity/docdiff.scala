...
def clean(s: String) : List[String] =   {
    ...
    val allWords = word_pattern.findAllIn(s).toList
...
def occurrences(xs: List[String]): Map[String, Int] =   {
    val uniqueSet = xs.toSet
    uniqueSet.map(word => (word, xs.count(_ == word))).toMap
...
def prod(lst1: List[String], lst2: List[String]) : Int =    {
    ...
    val commonSet = occ1.keySet.intersect(occ2.keySet)

    val dotProd = commonSet.toList.map(word => occ1(word) * occ2(word))
...
def overlap(lst1: List[String], lst2: List[String]) : Double =  {
    val dp1 = prod(lst1, lst1).toDouble
    val dp2 = prod(lst2, lst2).toDouble

    val result = prod(lst1, lst2).toDouble / dp1.max(dp2)
