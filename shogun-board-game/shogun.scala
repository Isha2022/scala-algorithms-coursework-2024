...
@main def main(): Unit = {
  ...
  // println(attacked(Red, b2)) //== Set(Pawn(2,Wht,(8,4)), King(2,Wht,(7,1)))
  // println(attacked(Wht, b2)) //== Set(Pawn(3,Red,(6,1)))
  // println(attacked(Wht, b_init)) //== Set()
  // println(attacked(Red, b_init)) //== Set()
  // println(all_moves(Pawn(3, Red, (6, 1)), b2))
...
def eval(pc: Piece, m: Move, en: Int, b: Board) : Set[Piece] =  {
  //Outsode board
  if (!inside(pc.pos, b)) Set()

  
  else if (en==0)  {
    occupied(pc.pos, b) match {
      case None => Set(pc) //energy 0 + pos not occupied
      //energy 0 + pos IS occupied
      case Some(piece) if piece.col != pc.col => Set(pc)
      case _ => Set()
    }
  }
  
  //energy > 0 + pos occupoed
  else if (is_occupied(pc.pos, b) && en>0)  {
    Set()
  }
  ...
  //analyse move m
    ...
    // case RU =>
    //   eval(incx(pc), RU, en-1, b) ++
    //   eval(pc, U, en, b)
    // case LU => 
    //   eval(decx(pc), LU, en- 1, b) ++ 
    //   eval(pc, U, en, b)
    // case RD => 
    //   eval(incx(pc), RD, en- 1, b) ++ 
    //   eval(pc, D, en, b)
    // case LD => 
    //   eval(decx(pc), LD, en- 1, b) ++ 
    //   eval(pc, D, en, b)
    // case UR => 
    //   eval(incy(pc), UR, en - 1, b) ++ 
    //   eval(pc, R, en, b)
    // case UL => 
    //   eval(incy(pc), UL, en - 1, b) ++ 
    //   eval(pc, L, en, b)
    // case DR => 
    //   eval(decy(pc), DR, en - 1, b) ++ 
    //   eval(pc, R, en, b)
    // case DL => 
    //   eval(decy(pc), DL, en - 1, b) ++ 
    //   eval(pc, L, en, b)

    case RU => 
      val right = if (!is_occupied(incx(pc).pos, b)) {
        eval(incx(pc), RU, en - 1, b)
      } else Set()
      val up = if (!is_occupied(incy(pc).pos, b)) {
        eval(pc, U, en, b)
      } else Set()
    ...
    case LU => 
      val left = if (!is_occupied(decx(pc).pos, b)) {
        eval(decx(pc), LU, en - 1, b)
      } else Set()
    ...
    case RD => 
      ...
      val down = if (!is_occupied(decy(pc).pos, b)) {
        eval(pc, D, en, b)
      } else Set()
...
def all_moves(pc: Piece, b: Board) : Set[Piece] = {
  val moves = Set(U,D,R,L,RU,LU,RD,LD,UR,UL,DR,DL)
  moves.flatMap(move => eval(pc,move,pc.en,b))
}


// Task 3: 
def attacked(c: Colour, b: Board) : Set[Piece] =  {
  // val ourPieces = b.pces.filter(_.col == c)
  // val oppositePieces = b.pces.filter(_.col != c)
  // val allPossibleMoves = ourPieces.flatMap(piece => all_moves(piece, b))
  // oppositePieces.filter(opp => 
  //   allPossibleMoves.exists(move => move.pos == opp.pos)
  // )

  val ourPieces = b.pces.filter(_.col == c)
  println(s"Our Pieces: $ourPieces")
  val allPossiblePositions = ourPieces.flatMap(piece => all_moves(piece, b).map(_.pos))
  println(s"All Possible Positions: $allPossiblePositions")
  val result = b.pces.filter(opp => opp.col != c && allPossiblePositions.contains(opp.pos))
  println(s"Attacked Pieces: $result")
